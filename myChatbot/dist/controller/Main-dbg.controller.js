sap.ui.define([
  ...
  return Controller.extend("sap.suite.ui.commons.demo.tutorial.controller.Main", {
    ...
    onItemSelect: function(oEvent) {
      ...
      if (sKey === "chat" || sKey === "reports" || sKey === "jokers") {
    ...
    _onMainMatched: function() {
      var sKey = this.getView().getModel("app").getProperty("/selectedMenuKey") || "chat";
      this._navigateByKey(sKey);
    },

    _onMainMenuMatched: function(oEvent) {
      var sKey = oEvent.getParameter("arguments").menuKey || "chat";
      if (sKey !== "chat" && sKey !== "reports" && sKey !== "jokers") {
        sKey = "chat";
      }
    ...
    _navigateByKey: function(sKey) {
      ...
      var oPage = oPageMap[sKey] || oPageMap.chat;
      if (oPage) {
        oNav.to(oPage);
      }
    }
